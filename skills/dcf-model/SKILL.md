---
name: dcf-model
description: Real DCF (Discounted Cash Flow) model creation for equity valuation. Retrieves financial data from SEC filings and analyst reports, builds comprehensive cash flow projections with proper WACC calculations, performs deterministic sensitivity analysis and 1,000-iteration Monte Carlo simulation, and outputs professional four-sheet Excel models (DCF, WACC, Sensitivity Analysis, Monte Carlo) with executive summaries. Use when users need to value a company using DCF methodology, request intrinsic value analysis, or ask for detailed financial modeling with growth projections and terminal value calculations.
---
...
This skill creates institutional-quality DCF models for equity valuation following investment banking standards. Each analysis produces a detailed Excel model with four sheets: DCF, WACC, Sensitivity Analysis, and Monte Carlo.
...
- After terminal value + PV → show the equity bridge (EV → equity value → per share), confirm before sensitivity analysis
- Catch errors at each stage — a wrong margin assumption discovered after sensitivity analysis is built means rebuilding everything downstream

**Sensitivity Analysis:**
- Keep the three deterministic 2D sensitivity tables as mandatory outputs.
- Add a Monte Carlo simulation as a supplemental sensitivity method, not a replacement for the 2D tables.
...
- Use a reproducible fixed seed by default (e.g., `seed = 42`) and document the seed, iteration count, sampled variables, distribution type, and bounds.
- Default to **1,000 Monte Carlo iterations**. This keeps the workbook responsive while still producing a useful valuation distribution.
- Random sampled assumptions may be written as hardcoded generated inputs, but each iteration's implied share price must be a live Excel formula that recalculates the DCF from those sampled assumptions.
- Do NOT use volatile `RAND()` / `NORM.INV(RAND())` formulas unless the user explicitly wants a dynamic simulation; reproducible fixed-seed simulations are preferred for auditability.
- Enforce logical constraints in every iteration: terminal growth < WACC, margins within realistic bounds, tax rates non-negative and reasonable, and revenue growth assumptions consistent with the business model.
- Include a Monte Carlo summary table showing mean, median, standard deviation, P5, P25, P75, P95, and probability implied price exceeds current share price.
...
Build **three deterministic sensitivity tables** plus a **Monte Carlo simulation** showing how valuation changes with different assumptions:
...
4. **Monte Carlo Simulation** - Shows valuation distribution across thousands of randomized but bounded assumption sets

**Implementation**: The first three outputs are simple 2D grids on the **Sensitivity Analysis** sheet (NOT Excel's "Data Table" feature) with formulas in each cell. Each cell must contain a full DCF recalculation for that specific assumption combination. The Monte Carlo output goes on the **Monte Carlo** sheet and should use fixed-seed random sampling generated through Python/openpyxl, with sampled assumptions written into the workbook and valuation outputs calculated by Excel formulas. Include both iteration-level outputs and a summary distribution table. See Critical Constraints section for detailed requirements on populating all deterministic sensitivity cells and Monte Carlo outputs programmatically.
...
### Correct Sensitivity Table and Monte Carlo Implementation
...
Monte Carlo simulation is a supplemental sensitivity method that shows the distribution of valuation outcomes. It must not replace the three deterministic 2D sensitivity tables.

**Recommended setup:**
- Use a fixed random seed for reproducibility (default: `seed = 42`)
- Default to 1,000 iterations
- Create sampled assumption columns for the key valuation drivers
- Use live Excel formulas for each iteration's DCF output and implied share price
- Add a summary table with distribution statistics and probability of upside

**Default sampled variables:**
- Revenue growth by projection year
- EBIT margin by projection year
- WACC or WACC drivers (beta, risk-free rate, equity risk premium)
- Terminal growth rate
- CapEx % of revenue
- NWC % of revenue change

**Preferred distributions:**
- Use triangular distributions where Bear/Base/Bull assumptions exist: `triangular(low=bear, mode=base, high=bull)`
- Use bounded normal distributions only when a defensible mean and standard deviation are available
- Enforce constraints after sampling: terminal growth < WACC, margins within realistic bounds, tax rates non-negative and reasonable
...
for iteration in range(1, 1001):
    revenue_growth = random.triangular(bear_growth, bull_growth, base_growth)
    ebit_margin = random.triangular(bear_margin, bull_margin, base_margin)
    wacc = random.triangular(bull_wacc, bear_wacc, base_wacc)
    terminal_growth = min(
        random.triangular(bear_terminal_g, bull_terminal_g, base_terminal_g),
        wacc - 0.005,
    )

    # Write sampled assumptions as generated inputs.
    # Write implied price as a live Excel formula using those sampled assumptions.
```

**Summary outputs:**
```csv
Metric,Formula
Mean Implied Price,=AVERAGE([simulation implied price range])
Median Implied Price,=MEDIAN([simulation implied price range])
Standard Deviation,=STDEV.S([simulation implied price range])
P5,=PERCENTILE.INC([simulation implied price range],0.05)
P25,=PERCENTILE.INC([simulation implied price range],0.25)
P75,=PERCENTILE.INC([simulation implied price range],0.75)
P95,=PERCENTILE.INC([simulation implied price range],0.95)
Probability of Upside,=COUNTIF([simulation implied price range],">"&[current price cell])/COUNT([simulation implied price range])
```

Document the random seed, iteration count, distribution choices, and bounds in the Monte Carlo section. The simulation must be reproducible and audit-ready.
...
1. **DCF** - Main valuation model, scenario assumptions, forecast, FCF build, terminal value, and valuation bridge
2. **WACC** - Cost of capital calculation
3. **Sensitivity Analysis** - Three deterministic 2D sensitivity tables
4. **Monte Carlo** - 1,000 fixed-seed simulation iterations and summary valuation distribution

**CRITICAL**: Do not place the deterministic sensitivity tables at the bottom of the DCF sheet. Put them on the dedicated `Sensitivity Analysis` sheet. Put Monte Carlo assumptions, 1,000 iteration-level valuation outputs, and distribution summary on the dedicated `Monte Carlo` sheet.
...
- **Do NOT use Excel's default red-yellow-green heatmap** for sensitivity tables or Monte Carlo outputs unless the user explicitly requests it. It looks generic and clashes with the model's professional blue/grey palette.
...
- For sensitivity tables, use a restrained blue/grey three-color scale:
  - Low values: very light blue-grey (`#F7F9FB`)
  - Mid values: light blue (`#D9E1F2`)
  - High values: dark blue (`#1F4E79`)
- Keep the base-case center cell distinct with medium blue (`#BDD7EE`), bold font, and a clear border.
- For Monte Carlo implied price outputs, prefer percentile band formatting instead of a full heatmap:
  - `< P5`: light blue-grey (`#E7EEF4`)
  - `P5-P25`: very light grey (`#F4F6F8`)
  - `P25-P75`: white
  - `P75-P95`: light blue (`#D9E1F2`)
  - `> P95`: medium blue (`#9DC3E6`)
- Alternative for Monte Carlo implied price: use a subtle blue data bar while keeping cell fills white.
...
Risk-Free Rate (10Y Treasury),X.XX%,[Input cell]
Beta (5Y monthly),X.XX,[Input cell]
Equity Risk Premium,X.XX%,[Input cell]
Cost of Equity,X.XX%,[Calculated blue]
,,
COST OF DEBT CALCULATION,,
Credit Rating,AA-,[Input cell]
Pre-Tax Cost of Debt,X.XX%,[Input cell]
...
,,
Total Debt ($M),XXX,[Input cell]
Cash & Equivalents ($M),XXX,[Input cell]
...
**Location**: Dedicated `Sensitivity Analysis` sheet. Do not put these tables at the bottom of the DCF sheet.
...
**Minimum deterministic sensitivity formulas to write: 75** (this is required, not optional; Monte Carlo iteration formulas are incremental)
...
4. Format cells with conditional formatting using the blue/grey three-color scale from Formatting Standards: low `#F7F9FB`, mid `#D9E1F2`, high `#1F4E79`
...
**Location**: Dedicated `Monte Carlo` sheet.

**Required output**: 1,000 DCF simulation rows plus a summary distribution table.

**Monte Carlo setup:**
1. Use a fixed seed (default: `seed = 42`) so the simulation is reproducible.
2. Generate 1,000 randomized but bounded assumption sets.
3. Use Bear/Base/Bull scenario assumptions as triangular distribution bounds where available.
4. Each iteration must calculate a full DCF-derived implied share price.
5. Include summary outputs: mean, median, standard deviation, P5, P25, P75, P95, and probability implied price exceeds current share price.

**Monte Carlo formatting:**
- Hide intermediate helper/random columns such as `u Rev Y1`, `u EBIT`, `u WACC`, and other uniform random draw columns.
- Keep sampled assumption columns visible only if they help audit the simulation; otherwise group or hide them.
- Format the final `Implied Price` column using percentile band formatting from Formatting Standards, not red-yellow-green heatmaps.
- Keep the summary distribution table near the top or right side of the sheet with the same blue/grey header and output-row styling used elsewhere in the workbook.

**No volatile formulas by default**: Prefer fixed-seed Python/openpyxl generated inputs or deterministic non-volatile formulas. Use `RAND()` only if the user explicitly asks for a dynamic simulation.
...
**Four sheets**:
1. **DCF** - Complete model with Bear/Base/Bull cases, forecast, FCF build, terminal value, and valuation bridge
2. **WACC** - Cost of capital calculation
3. **Sensitivity Analysis** - Three deterministic 2D sensitivity tables (WACC vs Terminal Growth, Revenue Growth vs EBIT Margin, Beta vs Risk-Free Rate)
4. **Monte Carlo** - 1,000 fixed-seed DCF simulation valuation results and distribution summary

**Key features**: Case selector (1/2/3), consolidation column with INDEX/OFFSET formulas, color-coded cells, cell comments on all inputs, professional borders, deterministic sensitivity tables, reproducible 1,000-iteration Monte Carlo simulation
...
1. **Verify structure**:
   - Four sheets present: DCF, WACC, Sensitivity Analysis, Monte Carlo
   ...
   - Three deterministic sensitivity tables on the `Sensitivity Analysis` sheet
   - Monte Carlo sheet includes 1,000 simulation rows and summary distribution outputs
   - Font colors: Blue inputs, black formulas, green sheet links
   - Visual formatting uses the professional blue/grey palette; no default red-yellow-green heatmaps unless explicitly requested
...
- Four sheets: DCF, WACC, Sensitivity Analysis, Monte Carlo
- Font colors: Blue=inputs, Black=formulas, Green=sheet links
- Cell comments on ALL hardcoded inputs
- Sensitivity Analysis sheet fully populated with three deterministic formula-driven sensitivity tables
- Monte Carlo sheet includes 1,000 DCF simulation valuation results and a summary distribution table
- Sensitivity and Monte Carlo outputs use blue/grey conditional formatting, with Monte Carlo implied price formatted by percentile bands or subtle blue data bars
- Professional borders around major sections
